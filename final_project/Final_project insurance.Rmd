...
X <- model.matrix(policy.output~., data.train)[,-1]
dim(X)
#View(X)
...
```{r}
# Next, we use glm() with the variables obtained from LASSO above. Also we will bring back each factor as an input instead of factor levels output from LASSO. For example "Customer.main.type4" and "Customer.main.type5" are returned by LASSO but we will input Customer.main.type into glm()
beta.min <- rownames(as.matrix(coef.min))
beta.min

# Create the logistic regression summary
fit.logit.1 <- glm(policy.output~Customer.main.type+Customer.Subtype+Avg.age+Roman.catholic+Protestant+Other.religion+No.religion+Married+Living.together+Other.relation+Singles+Household.without.children+Household.with.children+High.level.education+Medium.level.education+Lower.level.education+High.status+Entrepreneur+Farmer+Middle.management+Skilled.labourers+Unskilled.labourers+Social.class.A+Social.class.B1+Social.class.B2+Social.class.C+Social.class.D+Rented.house+Home.owners+X1.car+No.car+National.Health.Service+Private.health.insurance+Income...30+Income.30.45.000+Income.45.75.000+Income.75.122.000+Income..123.000+Average.income+Purchasing.power.class+Contribution.private.third.party.insurance+Contribution.third.party.insurance..firms.+Contribution.third.party.insurane..agriculture.+Contribution.car.policies+Contribution.motorcycle.scooter.policies+Contribution.lorry.policies+Contribution.agricultural.machines.policies+Contribution.private.accident.insurance.policies+Contribution.family.accidents.insurance.policies+Contribution.fire.policies+Contribution.surfboard.policies+Contribution.boat.policies+Contribution.property.insurance.policies+Contribution.social.security.insurance.policies+No..of.3rd.party.insurane..agriculture.+No..of.delivery.van.policies+No..of.lorry.policies+No..of.tractor.policies+No..of.moped.policies+No..of.surfboard.policies+No..of.boat.policies+No..of.bicycle.policies, family=binomial, data=data.train)
summary(fit.logit.1)
```